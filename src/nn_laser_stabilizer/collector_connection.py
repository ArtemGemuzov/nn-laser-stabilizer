...
from nn_laser_stabilizer.collector_utils import CollectorCommand, CollectorError
...
class CollectorConnection:
    ...
    def send_worker_error(self, error: CollectorError) -> None:
    ...
    def wait_for_ready(self, timeout: Optional[float] = None) -> Optional[CollectorError]:
    ...
    def wait_for_weight_update_done(self, timeout: Optional[float] = None) -> Optional[CollectorError]:
    ...
    def wait_for_shutdown_complete(self, timeout: Optional[float] = None) -> Optional[CollectorError]:
    ...
    def poll_worker_error(self, timeout: Optional[float] = None) -> Optional[CollectorError]:
