...
def main(config: DictConfig) -> None:
    ...
    try: 
        ...
        for tensordict_data in collector:
            ...
            logger.info(f"[Environment] Average reward per step = {avg_reward:.4f} at frame {total_collected_frames}")

            max_episode_length = steps.max().item()
            logger.info(f"[Environment] Max episode length = {max_episode_length} at frame {total_collected_frames}")
