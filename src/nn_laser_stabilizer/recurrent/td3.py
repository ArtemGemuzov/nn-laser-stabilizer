...
class RecurrentTD3:
    def __init__(self, obs_dim, action_dim, 
                 ...
                 policy_freq=2, shared_summary=True, device='cpu'):
        ...
        if shared_summary:
            self.shared_summarizer = Summarizer(obs_dim, rnn_hidden_size, num_rnn_layers).to(device)
            self.shared_summarizer_target = make_target(self.shared_summarizer)
            
        if shared_summary:
            self.actor_summarizer = self.shared_summarizer
            self.actor_summarizer_target = self.shared_summarizer_target
        else:
        ...
        if shared_summary:
            self.critic1_summarizer = self.shared_summarizer
            self.critic1_summarizer_target = self.shared_summarizer_target
        else:
        ...
        if shared_summary:
            self.critic2_summarizer = self.shared_summarizer
            self.critic2_summarizer_target = self.shared_summarizer_target
        else:
        ...
        if shared_summary:
            self.shared_optimizer = self.make_optimizer(self.shared_summarizer)
            self.actor_mlp_optimizer = self.make_optimizer(self.actor_mlp)
            self.critic1_mlp_optimizer = self.make_optimizer(self.critic1_mlp)
            self.critic2_mlp_optimizer = self.make_optimizer(self.critic2_mlp)
        else:
    ...
    def train_step(self, replay_buffer, batch_size=32, seq_len=10):
        ...
        with torch.no_grad():
            ...
            if self.shared_summary:
                target_critic_summary, _ = self.shared_summarizer_target(next_observations)
                target_q1 = self.critic1_mlp_target(target_critic_summary, next_actions)
                target_q2 = self.critic2_mlp_target(target_critic_summary, next_actions)
            else:
        ...
        if self.shared_summary:
            current_summary, _ = self.shared_summarizer(observations)
            current_q1 = self.critic1_mlp(current_summary, actions)
            current_q2 = self.critic2_mlp(current_summary, actions)
        else:
        ...
        if self.shared_summary:
            ...
            self.critic1_mlp_optimizer.zero_grad(set_to_none=True)
            self.critic2_mlp_optimizer.zero_grad(set_to_none=True)
            
            critic1_loss.backward(retain_graph=True)
            critic2_loss.backward()
            
            self.shared_optimizer.step()
            self.critic1_mlp_optimizer.step()
            self.critic2_mlp_optimizer.step()
        ...
        if self.total_it % self.policy_freq == 0:
            if self.shared_summary:
                actor_summary, _ = self.shared_summarizer(observations)
                actor_actions = self.actor_mlp(actor_summary)
                
                actor_q1 = self.critic1_mlp(actor_summary.detach(), actor_actions)
            else:
            ...
            if self.shared_summary:
                self.shared_optimizer.zero_grad(set_to_none=True)
                self.actor_mlp_optimizer.zero_grad(set_to_none=True)
                
                actor_loss.backward()
                
                self.shared_optimizer.step()
                self.actor_mlp_optimizer.step()
            else:
            ...
            if self.shared_summary:
                self.soft_update(self.shared_summarizer, self.shared_summarizer_target)
            else:
