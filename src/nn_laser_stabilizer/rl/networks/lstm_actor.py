...
class LSTMActorNetwork(ActorNetwork):
    def __init__(
        ...
        self.mlp = build_mlp(lstm_hidden_size, output_dim, mlp_hidden_sizes)
    ...
    def forward(self, obs: Tensor, state: HiddenState | None = None) -> NetworkOutput:
        ...
        output_flat = self.mlp(lstm_flat)  # (batch_size * seq_len, output_dim)
        output = output_flat.reshape(batch_size, seq_len, -1)  # (batch_size, seq_len, output_dim)

        if was_1d:
            output = output.squeeze(0).squeeze(0)  # (output_dim)
        elif was_2d:
            output = output.squeeze(1)  # (batch_size, output_dim)

        return NetworkOutput(output=output, state=new_state)
    ...
    def clone(self, reinitialize_weights: bool = False) -> "LSTMActorNetwork":
        new_net = LSTMActorNetwork(
            self._obs_dim, self._output_dim,
